...
three_digits_set = { (i*base*base+j*base+k) for i in digits for j in digits for k in digits }
...
## Task 18
odd_num_list_range = {i for i in range(0, 100, 2)}
...
k = 'Bilbo'
value_list_modified_1 = [d[k] for d in dlist] # <-- Use the same expression here
k = 'Frodo'
...
base = 10
digits = set(range(10))
representation_dict = {i*base*base+j*base+k:[i,j,k] for i in digits for j in digits for k in digits}

## Task 26
d = {0:1000.0, 1:1200.50, 2:990}
names = ['Larry', 'Curly', 'Moe']
listdict2dict = { n:d[i] for i,n in enumerate(names) if i in d}
