...
def makerandomtree(pc, maxdepth=4, fpr=.5, ppr=.6):
...
def crossover(t1, t2, probswap=.7, top=1):
...
def evolve(pc, popsize, rankfunction, maxgen=500, mutationrate=.1, breedingrate=.4, pexp=.7, pnew=.05):
    def selectindex():
...
def main():
    ...
    evolve(2, 500, rf, mutationrate=.2, breedingrate=.1, pexp=.7, pnew=.1)
...
def gridgame(p):
    ...
    for o in range(50):
        for i in range(2):
            ...
            if move == 1:
                location[i][0] += 1
                if location[i][1] > max[0]: location[i][0] = max[0]
            if move == 2:
                location[i][1] -= 1
